...
def generate_hate_speech_dataset(num_samples=100, output_file='hate_speech_evaluation_dataset.csv'):
    """
...
# Run the function to generate a dataset with 200 samples
if __name__ == "__main__":
    # Generate a timestamp for the filename
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"hate_speech_evaluation_dataset_{timestamp}.csv"
    
    # Generate the dataset
    df = generate_hate_speech_dataset(num_samples=200, output_file=filename)
